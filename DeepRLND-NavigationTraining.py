...
def dqn(env, n_episodes=2000, max_t=1000, eps_start=1.0, eps_end=0.01, eps_decay=0.995):
    ...
    brain_name = env.brain_names[0]                        # environment details
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        if np.mean(scores_window)>=13.0:                   #Checking if environment has been solved
            ...
            torch.save(agent.qnetwork_local.state_dict(), "/".join([projectFilesPath,'results',time.strftime("%Y%m%d-%H%M%S"),'solution.pth']))
            break
    return scores
...
def DemoTrainedAgent(env, agentFile, demoEpisodes=3, max_t=200):
    ...
    action_size = brain.vector_action_space_size
    state_size = len(env_info.vector_observations[0])
    state_dict = torch.load(agentFile)                     # loading agent details from file
    agent = Agent(state_size, action_size,seed=0)          # initialize agent
    agent.setFClayerNeurons(fc1_units=state_dict['fc1.weight'].shape[0], fc2_units=state_dict['fc2.weight'].shape[0]) # update Q-network architecture to match loaded agent
    agent.qnetwork_local.load_state_dict(torch.load(agentFile)) # initialize Q-network with loaded weights
    for i_episode in range(1,demoEpisodes+1):
        ...
        if i_episode % 100 == 0:                           #Taking an average score every 100 episodes
            print('\rEpisode {}\tAverage Score: {:.2f}'.\
                  format(i_episode, np.mean(scores_window)))
        if np.mean(scores_window)>=15.0:                   #Checking if environment has been solved
        ...
        if response == "train":
            ...
            while True:
                ...
                if not os.path.exists(agentFile):
                    ...
                    break
            filename = "demo"
            scores = DemoTrainedAgent(env, agentFile, demoEpisodes=100)
            break
        else:
